  ...
  computed: {
    ...
    isMobile() {
      ...
      return (
        ...
        'screen' in window &&
        (window.screen.height < mobileResolution || window.screen.width < mobileResolution)
      )
    }
  },
  watch: {
    ...
    this.initConfig()
      ...
      .then(() =>
        this.$store.state.isRunningEmbedded
          ? this.$store.dispatch('sendMessageToParentWindow', { event: 'ready' })
          : Promise.resolve()
      )
      .then(() => {
    ...
    messageHandler(evt) {
      ...
      switch (evt.data.event) {
        ...
        case 'setSessionAttribute':
          ...
          this.$store
            .dispatch('setSessionAttribute', { key: evt.data.key, value: evt.data.value })
            .then(() =>
              evt.ports[0].postMessage({
                event: 'resolve',
                type: evt.data.event
